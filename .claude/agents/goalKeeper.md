...
4. Agent(readonly: true, prompt: $EVAL_PROMPT)   ← Claude Code subagent tool
...
- **Subagent tool:** `Agent` — not `Task`. Example: `Agent(readonly: true, prompt: $EVAL_PROMPT)`
- **Nesting:** Subagents cannot spawn other subagents (1 depth max). The evaluator must return a verdict directly; goalKeeper handles the loop.
- **Stop hook:** Configure `goal-stop.sh` in `CLAUDE.md` (not `hooks.json`).
